# this program loads Census ACS data using unlogged tables,temporary tables and memory tuning
import time
import psycopg2
import argparse
import csv
...
def createStagingTable(conn):
    with conn.cursor() as cursor:
        cursor.execute(f"""
            ...
            CREATE TEMPORARY  TABLE {StagingTableName} (  #CREATE UNLOGGED  TABLE
                CensusTract         NUMERIC,
                State               TEXT,
                County              TEXT,
                TotalPop            INTEGER,
        ...
        """)
        print(f"Created {StagingTableName}")

# load data from staging table to main table and create index/constraints
def appendData(conn):
    with conn.cursor() as cursor:
        cursor.execute(f"""
            INSERT INTO {TableName} SELECT * FROM {StagingTableName};
            ALTER TABLE {TableName} ADD PRIMARY KEY (CensusTract);
            CREATE INDEX idx_{TableName}_State ON {TableName}(State);
        """)
        print(f"Appended data to {TableName} from {StagingTableName}")
...
def main():
    ...
    cmdlist = getSQLcmnds(rlis, StagingTableName)
